...
import com.example.erp.oms.accounting.domain.TaxAuthorityRateType;
import com.example.erp.oms.accounting.repository.TaxAuthorityRateTypeRepository;
...
public class TaxAuthorityRateTypeService {
    ...
    public TaxAuthorityRateType get(Long id) {
        return repository.findById(id).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "TaxAuthorityRateType %d not found".formatted(id)));
    }
