...
import com.example.erp.oms.accounting.repository.ProductAverageCostRepository;
...
public class ProductAverageCostService {
    ...
    public ProductAverageCost get(Long id) {
        return repository.findById(id).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "ProductAverageCost %d not found".formatted(id)));
    }
