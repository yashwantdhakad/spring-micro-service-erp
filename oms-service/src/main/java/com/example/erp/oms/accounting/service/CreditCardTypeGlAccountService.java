...
import com.example.erp.oms.accounting.domain.CreditCardTypeGlAccount;
import com.example.erp.oms.accounting.repository.CreditCardTypeGlAccountRepository;
...
public class CreditCardTypeGlAccountService {
    ...
    public CreditCardTypeGlAccount get(Long id) {
        return repository.findById(id).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "CreditCardTypeGlAccount %d not found".formatted(id)));
    }
