...
import com.example.erp.oms.accounting.repository.PaymentGatewayCyberSourceRepository;
...
public class PaymentGatewayCyberSourceService {
    ...
    public PaymentGatewayCyberSource get(Long id) {
        return repository.findById(id).orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "PaymentGatewayCyberSource %d not found".formatted(id)));
    }
